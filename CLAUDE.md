...
- **Explore / Chart builder (`/explore/`) — partial, first pass 2026-08-16** — Bar, Line, Table, and Big Number are live with real preview (client-side aggregation of `sampleRows` from `src/data/datasets.ts`, not a mock image) and the four right-panel tabs (Data / Customize / Query / Results) with a reduced but real control set. Remaining 8 viz types from `ChartVizType` (Pie, Scatter, Heatmap, Area, Box Plot, Sunburst, Sankey, Gauge) render a "not yet implemented" placeholder instead of pretending to work. **Explicitly deferred — not built thin:** annotations, saved/ad-hoc metric builder UI, color-scheme picker beyond the default `chart-1..5` OKLCH palette, embed/share. Keep the deferral honest rather than shipping non-functional controls.
...
- **Remaining per spec** — Alerts/Reports, Annotation Layers, CSS Templates, Tags, Import/Export, Admin (Users / Roles / Permissions / RLS), Action Log, About/Health, Dashboard View/Edit. Phase 1 not yet done globally; Data and Semantic Layer + Explore (first pass) are done.
