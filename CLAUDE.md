...
- **Done** — Dashboard List, Chart List, SQL Lab (with compact History/Saved previews), Database List + Editor, Dataset List + Editor, Saved Queries standalone, Query History standalone, Upload CSV / Excel — the full **Data and Semantic Layer** per the reference spec is now closed. Single canonical seed remains `src/data/databases.ts` (supplemented by `src/data/sqllab.ts` derived view, `src/data/datasets.ts` hand-authored against real seeds, and `routes/api/uploads` in-memory mutation).
- **Remaining per spec** — Alerts/Reports, Annotation Layers, CSS Templates, Tags, Import/Export, Admin (Users / Roles / Permissions / RLS), Action Log, About/Health, Dashboard View/Edit, Chart Explore. Phase 1 not yet done globally; Data and Semantic Layer is done.
