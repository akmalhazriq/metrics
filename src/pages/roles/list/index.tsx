...
export default function RolesListPage() {
    ...
    try {
      ...
      const msg =
        e instanceof ApiError
          ...
          : e instanceof Error
            ? e.message
            : "We couldn't load roles. Try refreshing.";
      setError(msg);
      showToast(msg);
    } finally {
      setLoading(false);
  ...
  const handleBulkDelete = async () => {
    ...
    if (ok && !fail) showToast(`Deleted ${ok} roles`);
      ...
      showToast(`Deleted ${ok} of ${ok + fail} roles. ${fail} failed: ${lastErr}`);
  ...
  const handleSave = async (payload: Record<string, unknown>) => {
    if (editing) {
      const r = await fetch(`/api/roles/${editing.id as number}`, {
        method: "PUT",
      ...
      showToast(`Role "${String(payload.name)}" created`);
      setPage(1);
    }
    fetchList();
  };
    ...
    <AppShell>
      <div className="mx-auto max-w-[1280px] px-4 py-6 sm:px-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1 max-w-[60ch] text-sm leading-relaxed">
              Group permissions into roles. Assign roles to users to grant access.
            </p>
          </div>
          <Button size="sm" onClick={openCreate}>
            <Shield className="mr-1.5 h-3.5 w-3.5" /> Add role
          </Button>
        </div>
                    ...
                    <td colSpan={6} className="px-6 py-16 text-center">
                      <div className="mx-auto max-w-sm">
                        ...
                        <p className="text-muted-foreground mt-1 text-sm">
                          Create a role to bundle permissions.
                        </p>
                  ...
                  rows.map((r) => (
                    ...
                    >
                      ...
                      <td className="hidden px-2 py-2.5 lg:table-cell">
                        <div className="flex max-w-[42ch] flex-wrap gap-1">
                          {r.permNames.length === 0 ? (
                            <span className="text-muted-foreground text-[11px]">—</span>
                          ) : (
                            <>
                              {r.permNames.slice(0, 4).map((x) => (
                                <Badge key={x} variant="secondary" className="text-[10px]">
                                  {x}
                                </Badge>
                              ))}
                              {r.permNames.length > 4 && (
                        ...
                        <span className="text-muted-foreground text-[11px]">
                          {r.permCount} permission{r.permCount === 1 ? "" : "s"}
                        </span>
                      </td>
        ...
        <p className="text-muted-foreground mt-3 text-xs">
          Data via <code className="bg-muted rounded px-1 py-0.5">/api/roles</code> — Postgres +
          Drizzle.
        </p>
      </div>
      <RoleEditor
