...
type RoleRow = { id: number; name: string; description: string | null; permCount: number; permNames: string[]; userCount: number; createdAt: string; modifiedAt: string };
...
export default function RolesListPage() {
  ...
  const [rows, setRows] = useState<RoleRow[]>([]); const [total, setTotal] = useState(0); const [loading, setLoading] = useState(true);
  ...
  const fetchList = async () => {
    ...
    setError(null);
    try {
      const res = await apiFetchList<RoleRow>("/api/roles", {
        q: q || undefined,
        sortBy,
        sortDir,
        page,
      ...
      const msg = e instanceof ApiError ? e.message : e instanceof Error ? e.message : "Could not load Roles";
      setError(msg);
      showToast(msg);
    } finally {
      setLoading(false);
  ...
  useEffect(() => { fetchList(); }, [q, sortBy, sortDir, page, pageSize]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleDelete = async (row: RoleRow) => {
    try {
      await mutate(`/api/roles/${row.id}`, "DELETE");
  ...
  const handleBulkDelete = async () => {
    ...
    for (const id of ids) {
      try {
        await mutate(`/api/roles/${id}`, "DELETE");
        ok++;
      } catch (e) {
        fail++;
    ...
    if (ok && !fail) showToast(`Deleted ${ok} roles`);
    else if (ok && fail) showToast(`Deleted ${ok} of ${ok + fail} roles — ${fail} failed: ${lastErr}`);
  ...
  const handleSave = async (payload: Record<string, unknown>) => {
    if (editing) {
      const r = await fetch(`/api/roles/${editing.id as number}`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
      if (!r.ok) { const j = await r.json() as { error?: string }; showToast(j.error ?? "Save failed"); return; }
      showToast(`Role "${String(payload.name)}" saved`);
    } else {
      ...
      showToast(`Role "${String(payload.name)}" created`); setPage(1);
    }
    fetchList();
  };
  const openCreate = () => { setEditing(null); setEditorOpen(true); };
  const openEdit = (row: RoleRow) => { setEditing(row as unknown as Record<string, unknown>); setEditorOpen(true); };
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-[1280px] px-4 py-6 sm:px-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-[22px] font-semibold tracking-tight">Roles</h1>
              <span className="bg-muted text-muted-foreground rounded-full px-2 py-0.5 text-xs font-medium">{total}</span>
            </div>
            <p className="text-muted-foreground mt-1 max-w-[60ch] text-sm leading-relaxed">Group permissions into roles. Assign roles to users to grant access.</p>
          </div>
          <Button size="sm" onClick={openCreate}><Shield className="mr-1.5 h-3.5 w-3.5" /> Add role</Button>
        </div>
        ...
        <div className="border-border bg-card mt-6 rounded-lg border">
          ...
          <div className="border-border flex items-center gap-2 border-t px-3 py-2">
            <span className="text-muted-foreground text-xs">{loading ? "Loading…" : `${total} roles`}</span>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-border bg-muted/40 text-muted-foreground border-b text-left text-xs font-medium tracking-wide">
                  ...
                  <th className="hidden px-2 py-2.5 lg:table-cell">Permissions</th>
              ...
              <tbody className="divide-border divide-y">
                {loading ? Array.from({ length: 4 }).map((_, i) => (
                  <tr key={i} className="animate-pulse"><td className="px-3 py-3"><span className="bg-muted block h-3 w-3 rounded" /></td><td className="px-2 py-3"><span className="bg-muted block h-3 w-20 rounded" /></td><td colSpan={4} className="px-2 py-3"><span className="bg-muted block h-3 w-32 rounded" /></td></tr>
                )) : rows.length === 0 ? (
                  <tr><td colSpan={6} className="px-6 py-16 text-center"><div className="mx-auto max-w-sm"><p className="text-sm font-medium">No roles match your filters</p><p className="text-muted-foreground mt-1 text-sm">Create a role to bundle permissions.</p><div className="mt-4 flex justify-center"><Button size="sm" onClick={openCreate}>Add role</Button></div></div></td></tr>
                ) : rows.map((r) => (
                  <tr key={r.id} className={`group hover:bg-muted/40 ${selected.has(r.id) ? "bg-muted/60" : ""}`}>
                    ...
                    <td className="hidden px-2 py-2.5 lg:table-cell">
                      <div className="flex flex-wrap gap-1 max-w-[42ch]">
                        {r.permNames.length === 0 ? <span className="text-muted-foreground text-[11px]">—</span> : (
                          <>
                            {r.permNames.slice(0, 4).map((x) => <Badge key={x} variant="secondary" className="text-[10px]">{x}</Badge>)}
                            {r.permNames.length > 4 && <Badge variant="outline" className="text-[10px]">+{r.permNames.length - 4}</Badge>}
                          </>
                        )}
                      </div>
                      <span className="text-muted-foreground text-[11px]">{r.permCount} permission{r.permCount === 1 ? "" : "s"}</span>
                    </td>
                    <td className="px-2 py-2.5"><Badge variant="outline" className="text-[11px]">{r.userCount} user{r.userCount === 1 ? "" : "s"}</Badge></td>
        ...
        <p className="text-muted-foreground mt-3 text-xs">Data via <code className="bg-muted rounded px-1 py-0.5">/api/roles</code> — Postgres + Drizzle.</p>
      </div>
      <RoleEditor open={editorOpen} onClose={() => setEditorOpen(false)} initial={editing} onSave={handleSave} />
      <ConfirmDialog
        open={!!confirmRow}
      ...
      <ConfirmDialog
        ...
        description={`Delete ${selected.size} roles? This cannot be undone.`}
