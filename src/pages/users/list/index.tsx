...
type UserRow = {
  ...
  active: boolean; roles: string[]; databaseAccessCount: number; datasourceAccessCount: number;
  createdAt: string; modifiedAt: string;
};

export default function UsersListPage() {
  const [q, setQ] = useState(""); const [active, setActive] = useState("all"); const [role, setRole] = useState("all");
  ...
  const [rows, setRows] = useState<UserRow[]>([]); const [total, setTotal] = useState(0); const [loading, setLoading] = useState(true);
  ...
  const fetchList = async () => {
    ...
    try {
      ...
      const msg = e instanceof ApiError ? e.message : e instanceof Error ? e.message : "Could not load Users";
      setError(msg);
      showToast(msg);
    } finally {
      setLoading(false);
  ...
  useEffect(() => { void fetchList(); }, [q, active, role, sortBy, sortDir, page, pageSize]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleBulkDelete = async () => {
    ...
    else if (ok && fail) showToast(`Deleted ${ok} of ${ok + fail} users — ${fail} failed: ${lastErr}`);
  ...
  const handleSave = async (payload: Record<string, unknown>) => {
    if (editing) {
      const r = await fetch(`/api/users/${editing.id as number}`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
      ...
      showToast(`User "${String(payload.username)}" created`); setPage(1);
    }
    fetchList();
  };
  const openCreate = () => { setEditing(null); setEditorOpen(true); };
  const openEdit = (row: UserRow) => { setEditing(row as unknown as Record<string, unknown>); setEditorOpen(true); };
      ...
      <div className="mx-auto max-w-[1280px] px-4 py-6 sm:px-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1 max-w-[60ch] text-sm leading-relaxed">Manage platform users, their roles and data access.</p>
          </div>
          <Button size="sm" onClick={openCreate}><Shield className="mr-1.5 h-3.5 w-3.5" /> Add user</Button>
        </div>
        ...
        <div className="border-border bg-card mt-6 rounded-lg border">
          <div className="flex flex-col gap-3 p-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative flex-1 sm:max-w-[360px]">
              ...
              <Input placeholder="Search username, name, email…" value={q} onChange={(e) => { setQ(e.target.value); setPage(1); }} className="h-8 pl-8 text-sm" />
            </div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="relative">
                <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="border-input bg-background h-8 rounded-md border px-2 pr-6 text-xs font-medium">
                  <option value="modified">Sort: Modified</option><option value="username">Sort: Username</option><option value="email">Sort: Email</option>
                </select>
                <ChevronDown className="text-muted-foreground pointer-events-none absolute top-1/2 right-1.5 h-3.5 w-3.5 -translate-y-1/2" />
              </div>
          ...
          <div className="border-border flex flex-wrap items-center gap-2 border-t px-3 py-2">
            <span className="text-muted-foreground text-xs">{loading ? "Loading…" : `${total} users`}</span>
            {(q || active !== "all" || role !== "all") && <button onClick={() => { setQ(""); setActive("all"); setRole("all"); setPage(1); }} className="border-input bg-background ml-auto inline-flex items-center gap-1 rounded-md border px-2 py-1 text-xs font-medium"><X className="h-3 w-3" /> Clear</button>}
          </div>
          {selected.size > 0 && (
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-border divide-y">
                {loading ? Array.from({ length: 4 }).map((_, i) => (
                  ...
                  <tr><td colSpan={8} className="px-6 py-16 text-center"><div className="mx-auto max-w-sm"><p className="text-sm font-medium">No users match your filters</p><p className="text-muted-foreground mt-1 text-sm">Create a user to grant dashboard access.</p><div className="mt-4 flex justify-center"><Button size="sm" onClick={openCreate}>Add user</Button></div></div></td></tr>
                ) : rows.map((r) => (
                  <tr key={r.id} className={`group hover:bg-muted/40 ${selected.has(r.id) ? "bg-muted/60" : ""}`}>
                    <td className="px-3 py-3"><Checkbox checked={selected.has(r.id)} onChange={(e) => { const c = (e.target as HTMLInputElement).checked; setSelected((prev) => { const n = new Set(prev); if (c) n.add(r.id); else n.delete(r.id); return n; }); }} aria-label={`Select ${r.username}`} /></td>
                    <td className="px-2 py-3"><button onClick={() => openEdit(r)} className="text-left text-sm font-medium hover:underline">{r.username}</button><div className="text-muted-foreground text-[11px] sm:hidden">{r.firstName} {r.lastName}</div></td>
                    <td className="hidden px-2 py-2.5 sm:table-cell"><span className="text-xs">{r.firstName} {r.lastName}</span></td>
                    <td className="hidden px-2 py-2.5 lg:table-cell"><span className="text-xs">{r.email}</span></td>
                    <td className="px-2 py-2.5"><Badge variant={r.active ? "success" : "muted"} className="text-[11px]">{r.active ? "Active" : "Inactive"}</Badge></td>
                    ...
                    <td className="hidden px-2 py-2.5 xl:table-cell"><span className="text-xs">{r.databaseAccessCount} db · {r.datasourceAccessCount} ds</span></td>
        ...
        <p className="text-muted-foreground mt-3 text-xs">Data via <code className="bg-muted rounded px-1 py-0.5">/api/users</code> — Postgres + Drizzle.</p>
      </div>
      <UserEditor open={editorOpen} onClose={() => setEditorOpen(false)} initial={editing} onSave={handleSave} />
      <ConfirmDialog
        open={!!confirmRow}
