...
export default function ReportListPage() {
    ...
    try {
      ...
      const msg =
        e instanceof ApiError
          ...
          : e instanceof Error
            ? e.message
            : "We couldn't load reports. Try refreshing.";
      setError(msg);
      showToast(msg);
    } finally {
      setLoading(false);
  ...
  const handleBulkDelete = async () => {
    ...
    if (ok && !fail) showToast(`Deleted ${ok} reports`);
      ...
      showToast(`Deleted ${ok} of ${ok + fail} reports. ${fail} failed: ${lastErr}`);
    else if (!ok && fail) showToast(lastErr || "Could not delete. Try again.");
    setSelected(new Set());
    fetchList();
  };
  const handleTest = (payload: Record<string, unknown>) => {
    const n = Array.isArray(payload.recipients)
      ? (payload.recipients as string[]).length
  ...
  const handleSave = async (payload: Record<string, unknown>) => {
    if (editing) {
      ...
      showToast(`Report "${String(payload.name)}" created`);
      setPage(1);
    }
    fetchList();
  };
    ...
    <AppShell>
      <div className="mx-auto max-w-[1280px] px-4 py-6 sm:px-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1 max-w-[60ch] text-sm leading-relaxed">
              Scheduled exports of dashboards or charts — same delivery channels as alerts, without
              the condition.
            </p>
          </div>
          <Button
          ...
          >
            <FileText className="mr-1.5 h-3.5 w-3.5" /> Add report
          </Button>
        </div>
        ...
        <div className="border-border bg-card mt-6 rounded-lg border">
          ...
          <div className="border-border flex flex-wrap items-center gap-2 border-t px-3 py-2">
            <span className="text-muted-foreground hidden text-xs sm:inline">
              Pick dashboard or chart in the editor → filter values are frozen at send time.
            </span>
                    ...
                    <td colSpan={8} className="px-6 py-16 text-center">
                      <div className="mx-auto max-w-sm">
                        ...
                        <p className="text-muted-foreground mt-1 text-sm">
                          Reports snapshot a dashboard or chart on a schedule — no threshold needed.
                        </p>
                    ...
                    >
                      ...
                      <td className="px-2 py-3">
                        ...
                        <div className="text-muted-foreground text-[11px]">
                          {[
                            r.dashboardId ? `dashboard #${r.dashboardId}` : null,
                            r.chartId ? `chart #${r.chartId}` : null,
                          ]
                            .filter(Boolean)
                            .join(" · ") || "— no target —"}
                        </div>
                          ...
                          {openMenu === r.id && (
                            <div className="border-border bg-popover absolute top-8 right-0 z-20 w-56 rounded-md border p-1 shadow-lg">
                              ...
                              <button
                                ...
                                className="hover:bg-accent flex w-full items-center gap-2 rounded px-2 py-1.5 text-xs"
                              >
                                <FileText className="h-3.5 w-3.5" /> {r.active ? "Pause" : "Enable"}
                              </button>
                              <button
                                onClick={() => {
                                  setOpenMenu(null);
        ...
        <p className="text-muted-foreground mt-3 text-xs">
          Data via <code className="bg-muted rounded px-1 py-0.5">/api/reports</code> — Postgres +
          Drizzle.
        </p>
      </div>
      <AlertReportEditor
