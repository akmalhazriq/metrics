...
export default function DatasetEditAlias() {
      ...
      try {
        ...
        const msg =
          e instanceof ApiError
            ...
            : e instanceof Error
              ? e.message
              : "We couldn't load that dataset. Try refreshing.";
        setError(msg);
      } finally {
  ...
  if (loading)
    return (
      <AppShell>
        <div className="text-muted-foreground mx-auto max-w-[640px] px-6 py-12 text-sm">
          Loading dataset {id}…
        </div>
      </AppShell>
    );
    ...
    return (
      <AppShell>
        <div className="mx-auto max-w-[640px] px-6 py-12">
          ...
          <Link to="/datasets" className="mt-4 inline-flex text-xs underline">
            Back to datasets
          </Link>
        </div>
      </AppShell>
  ...
  if (!editing)
    return (
      <AppShell>
        <div className="mx-auto max-w-[640px] px-6 py-12 text-sm">Not found</div>
      </AppShell>
    );
    ...
    <AppShell>
      <div className="mx-auto max-w-[720px] px-4 py-6 sm:px-6">
        ...
        <p className="text-muted-foreground mt-1 text-xs">
          Hydrated via <code className="bg-muted rounded px-1">/api/datasets/{id}</code> — live
          databases from <code className="bg-muted rounded px-1">/api/databases</code>
        </p>

        <div className="border-border bg-card mt-6 space-y-4 rounded-lg border p-5">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
                  ...
                  onChange={(e) => {
                    ...
                    if (db)
                      setEditing({
                        ...
                        backend: db.backend as Dataset["backend"],
                        schema: db.schemas[0]?.name ?? editing.schema,
                        table: db.schemas[0]?.tables[0]?.name ?? editing.table,
                      });
                  }}
            ...
            <label className="space-y-1.5">
              ...
              <div className="relative">
                <select
                  value={editing.schema ?? "public"}
                  onChange={(e) => setEditing({ ...editing, schema: e.target.value })}
                  className="border-input bg-background h-9 w-full rounded-md border px-3 pr-8 text-sm"
                >
                  {(liveDbs.find((x) => x.id === editing.databaseId)?.schemas ?? []).map((s) => (
            ...
            <label className="space-y-1.5">
              ...
              <div className="relative">
                <select
                  ...
                  className="border-input bg-background h-9 w-full rounded-md border px-3 pr-8 font-mono text-xs"
                >
                  {(
                    liveDbs
