...
type RlsRow = {
  ...
  roles: string[];
  tables: { table: string; schema: string; databaseId: string }[];
  createdAt: string;
  modifiedAt: string;
};
...
export default function RlsListPage() {
    ...
    try {
      ...
      const msg =
        e instanceof ApiError
          ...
          : e instanceof Error
            ? e.message
            : "We couldn't load RLS filters. Try refreshing.";
      setError(msg);
      showToast(msg);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    void fetchList();
  }, [q, filterType, role, sortBy, sortDir, page, pageSize]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleBulkDelete = async () => {
    ...
    if (ok && !fail) showToast(`Deleted ${ok} RLS filters`);
      ...
      showToast(`Deleted ${ok} of ${ok + fail} RLS filters. ${fail} failed: ${lastErr}`);
  ...
  const handleSave = async (payload: Record<string, unknown>) => {
    if (editing) {
      const r = await fetch(`/api/rowlevelsecurity/${editing.id as number}`, {
        method: "PUT",
      ...
      const r = await fetch("/api/rowlevelsecurity", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(payload),
      });
      ...
      showToast(`Filter "${String(payload.name)}" created`);
      setPage(1);
    }
    fetchList();
  };
    ...
    <AppShell>
      <div className="mx-auto max-w-[1280px] px-4 py-6 sm:px-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1 max-w-[60ch] text-sm leading-relaxed">
              Filters applied per role to restrict rows visible in queries. Clause is a SQL WHERE
              condition.
            </p>
          </div>
          <Button size="sm" onClick={openCreate}>
            <Shield className="mr-1.5 h-3.5 w-3.5" /> Add filter
          </Button>
        </div>
        ...
        <div className="border-border bg-card mt-6 rounded-lg border">
          <div className="flex flex-col gap-3 p-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative flex-1 sm:max-w-[360px]">
              ...
              <Input
                placeholder="Search name, clause…"
                value={q}
                onChange={(e) => {
                  setQ(e.target.value);
                  setPage(1);
          ...
          <div className="border-border flex items-center gap-2 border-t px-3 py-2">
            <span className="text-muted-foreground text-xs">
              {loading ? "Loading…" : `${total} filters`}
            </span>
            {(q || filterType !== "all" || role !== "all") && (
              <button
                onClick={() => {
                  setQ("");
                  setFilterType("all");
                  ...
                  <tr>
                    <td colSpan={8} className="px-6 py-16 text-center">
                      <div className="mx-auto max-w-sm">
                        <p className="text-sm font-medium">No RLS filters match your filters</p>
                        <p className="text-muted-foreground mt-1 text-sm">
                          Create a clause to restrict rows per role.
                        </p>
                      ...
                      <td className="hidden px-2 py-2.5 lg:table-cell">
                        <div className="flex max-w-[28ch] flex-wrap gap-1">
                          {r.tables.length === 0 ? (
                            ...
                            r.tables.slice(0, 3).map((t) => (
                              <Badge
                                key={`${t.databaseId}.${t.schema}.${t.table}`}
                                variant="outline"
                                className="font-mono text-[10px]"
                              >
                                {t.databaseId}.{t.schema}.{t.table}
                              </Badge>
                            ))
                          )}
                          {r.tables.length > 3 && (
        ...
        <p className="text-muted-foreground mt-3 text-xs">
          Data via <code className="bg-muted rounded px-1 py-0.5">/api/rowlevelsecurity</code> —
          Postgres + Drizzle.
        </p>
      </div>
      <RlsEditor
