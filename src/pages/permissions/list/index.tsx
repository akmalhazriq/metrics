...
export default function PermissionsListPage() {
    ...
    try {
      ...
      const msg =
        e instanceof ApiError
          ...
          : e instanceof Error
            ? e.message
            : "We couldn't load permissions. Try refreshing.";
      setError(msg);
      showToast(msg);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    fetchList();
  }, [q, view, action, sortBy, sortDir, page, pageSize]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <AppShell>
      <div className="mx-auto max-w-[1280px] px-4 py-6 sm:px-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1 max-w-[60ch] text-sm leading-relaxed">
              View — action permissions generated by the platform. Read only. Assign them to roles.
            </p>
          </div>
        </div>
        ...
        <div className="border-border bg-card mt-6 rounded-lg border">
          <div className="flex flex-col gap-3 p-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative flex-1 sm:max-w-[360px]">
              ...
              <Input
                placeholder="Search name, view, action…"
                value={q}
                onChange={(e) => {
                  setQ(e.target.value);
                  setPage(1);
          ...
          <div className="border-border flex items-center gap-2 border-t px-3 py-2">
            <span className="text-muted-foreground text-xs">
              {loading ? "Loading…" : `${total} permissions`}
            </span>
            {(q || view !== "all" || action !== "all") && (
              <button
                onClick={() => {
                  setQ("");
                  setView("all");
                ...
                {loading ? (
                  Array.from({ length: 4 }).map((_, i) => (
                    <tr key={i} className="animate-pulse">
                      ...
                      <td className="px-2 py-3">
                        <span className="bg-muted block h-3 w-12 rounded" />
                      </td>
                    ...
                    <td colSpan={5} className="px-6 py-16 text-center">
                      <div className="mx-auto max-w-sm">
                        ...
                        <p className="text-muted-foreground mt-1 text-sm">
                          Permissions are generated from views — edit via roles.
                        </p>
                      </div>
                    </td>
                  </tr>
                  ...
                  rows.map((r) => (
                    <tr key={r.id} className="hover:bg-muted/40">
                      ...
                      <td className="px-2 py-2.5">
                        <Badge
                          variant={r.action === "write" ? "default" : "outline"}
                          className="text-[11px]"
                        >
                          {r.action}
                        </Badge>
        ...
        <p className="text-muted-foreground mt-3 text-xs">
          Data via <code className="bg-muted rounded px-1 py-0.5">/api/permissions</code> — read
          only.
        </p>
      </div>
      {toast && (
