...
export default function DatabaseEditAlias() {
      ...
      try {
        ...
        const msg =
          e instanceof ApiError
            ...
            : e instanceof Error
              ? e.message
              : "We couldn't load that database. Try refreshing.";
        setError(msg);
      } finally {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await mutate(`/api/databases/${editing.id}`, "PUT", {
        name: editing.name,
        sqlalchemyUri: editing.sqlalchemyUri,
    ...
    return (
      <AppShell>
        <div className="mx-auto max-w-[640px] px-6 py-12">
          ...
          <Link to="/databaseview/list" className="mt-4 inline-flex text-xs underline">
            Back to databases
          </Link>
        </div>
      </AppShell>
      ...
      <div className="mx-auto max-w-[720px] px-4 py-6 sm:px-6">
        <div className="flex items-center justify-between">
          <h1 className="text-[20px] font-semibold tracking-tight">
            Edit database · {editing.name}
          </h1>
          <Button variant="outline" size="sm" onClick={() => navigate("/databaseview/list")}>
            <X className="mr-1 h-3.5 w-3.5" /> Close
          </Button>
        </div>
        <p className="text-muted-foreground mt-1 text-xs">
          Hydrated via <code className="bg-muted rounded px-1">/api/databases/{id}</code> — live
          from Postgres
        </p>
        ...
        <div className="border-border bg-card mt-6 space-y-4 rounded-lg border p-5">
          ...
          <label className="block space-y-1.5">
            ...
            <Input
              ...
              onChange={(e) =>
                setEditing({ ...editing, backend: e.target.value as DatabaseConnection["backend"] })
              }
              className="h-9 text-sm"
            />
          </label>
          ...
          <div className="text-muted-foreground text-xs">
            Full editor with schemas/tables available at{" "}
            <Link to="/databaseview/list" className="underline">
              Database List → Edit
            </Link>
          </div>
