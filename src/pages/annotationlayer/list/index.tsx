...
import { ChevronDown, ChevronLeft, ChevronRight, ChevronsUpDown, Pencil, Search, StickyNote, Trash2, X } from "lucide-react";
...
import { AnnotationEditor } from "@/components/editors/AnnotationEditor";
import { fetchList as apiFetchList, ApiError, mutate } from "@/lib/api";

type Row = { id: number; name: string; description: string | null; annotationType: string; startField: string | null; endField: string | null; jsonMetadata: unknown; createdAt: string; modifiedAt: string };
...
export default function AnnotationListPage() {
  ...
  useEffect(() => { const h = (e: MouseEvent) => { if (menuRef.current && !menuRef.current.contains(e.target as Node)) setOpenMenu(null); }; document.addEventListener("mousedown", h); return () => document.removeEventListener("mousedown", h); }, []);

  const fetchList = async () => {
    setLoading(true);
    setError(null);
    try {
      const res = await apiFetchList<Row>("/api/annotationlayers", {
        q: q || undefined,
      ...
      const msg = e instanceof ApiError ? e.message : e instanceof Error ? e.message : "Could not load Annotation layers";
      setError(msg);
      showToast(msg);
    } finally {
      setLoading(false);
  ...
  useEffect(() => { fetchList(); }, [q, type, sortBy, sortDir, page, pageSize]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleDelete = async (row: Row) => {
    try {
      await mutate(`/api/annotationlayers/${row.id}`, "DELETE");
      showToast("Annotation layer deleted");
      fetchList();
    } catch (e) {
  ...
  const handleBulkDelete = async () => {
    ...
    for (const id of ids) {
      try {
        await mutate(`/api/annotationlayers/${id}`, "DELETE");
        ok++;
      } catch (e) {
        fail++;
    ...
    if (ok && !fail) showToast(`Deleted ${ok} annotation layers`);
    else if (ok && fail) showToast(`Deleted ${ok} of ${ok + fail} annotation layers — ${fail} failed: ${lastErr}`);
  ...
  const handleSave = async (payload: Record<string, unknown>) => {
    if (editing) {
      const r = await fetch(`/api/annotationlayers/${editing.id as number}`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
      if (!r.ok) { const j = await r.json() as { error?: string }; showToast(j.error ?? "Save failed"); return; }
      showToast(`Annotation layer "${String(payload.name)}" saved`);
    } else {
      const r = await fetch("/api/annotationlayers", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
      if (!r.ok) { const j = await r.json() as { error?: string }; showToast(j.error ?? "Create failed"); return; }
      showToast(`Annotation layer "${String(payload.name)}" created`); setPage(1);
    }
    fetchList();
  };
  const openCreate = () => { setEditing(null); setEditorOpen(true); };
  const openEdit = (row: Row) => { setEditing(row as unknown as Record<string, unknown>); setEditorOpen(true); };
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-[1280px] px-4 py-6 sm:px-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <div className="bg-muted text-muted-foreground grid h-7 w-7 place-items-center rounded-md"><StickyNote className="h-4 w-4" /></div>
              <h1 className="text-[22px] font-semibold tracking-tight">Annotation Layers</h1>
              <span className="bg-muted text-muted-foreground rounded-full px-2 py-0.5 text-xs font-medium">{total}</span>
            </div>
            <p className="text-muted-foreground mt-1 max-w-[60ch] text-sm leading-relaxed">Mark events and intervals on charts. Used by chart annotations to highlight releases, outages and targets.</p>
          </div>
          <Button size="sm" onClick={openCreate}><StickyNote className="mr-1.5 h-3.5 w-3.5" /> Add layer</Button>
        </div>
        ...
        <div className="border-border bg-card mt-6 rounded-lg border">
          <div className="flex flex-col gap-3 p-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              <div className="relative">
                <select value={type} onChange={(e) => { setType(e.target.value); setPage(1); }} className="border-input bg-background h-8 rounded-md border px-2 pr-6 text-xs font-medium">
                  <option value="all">All types</option><option value="time series">time series</option><option value="interval">interval</option><option value="event">event</option>
                </select>
              ...
              <div className="relative">
                <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="border-input bg-background h-8 rounded-md border px-2 pr-6 text-xs font-medium">
                  <option value="modified">Sort: Modified</option><option value="name">Sort: Name</option><option value="type">Sort: Type</option>
                </select>
                <ChevronDown className="text-muted-foreground pointer-events-none absolute top-1/2 right-1.5 h-3.5 w-3.5 -translate-y-1/2" />
              </div>
          ...
          <div className="border-border flex items-center gap-2 border-t px-3 py-2">
            <span className="text-muted-foreground text-xs">{loading ? "Loading…" : `${total} layers`}</span>
            {(q || type !== "all") && <button onClick={() => { setQ(""); setType("all"); setPage(1); }} className="border-input bg-background ml-auto inline-flex items-center gap-1 rounded-md border px-2 py-1 text-xs font-medium"><X className="h-3 w-3" /> Clear</button>}
          </div>
          {selected.size > 0 && (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-border bg-muted/40 text-muted-foreground border-b text-left text-xs font-medium tracking-wide">
                  ...
                  <th className="hidden px-2 py-2.5 sm:table-cell">Description</th>
                  <th className="px-2 py-2.5">Type</th>
                  <th className="hidden px-2 py-2.5 lg:table-cell">Start field</th>
                  <th className="hidden px-2 py-2.5 lg:table-cell">End field</th>
                  <th className="w-10 px-2 py-2.5"></th>
                </tr>
              </thead>
              <tbody className="divide-border divide-y">
                {loading ? Array.from({ length: 4 }).map((_, i) => (
                  <tr key={i} className="animate-pulse"><td className="px-3 py-3"><span className="bg-muted block h-3 w-3 rounded" /></td><td className="px-2 py-3"><span className="bg-muted block h-3 w-24 rounded" /></td><td colSpan={5} className="px-2 py-3"><span className="bg-muted block h-3 w-32 rounded" /></td></tr>
                )) : rows.length === 0 ? (
                  <tr><td colSpan={7} className="px-6 py-16 text-center"><div className="mx-auto max-w-sm"><p className="text-sm font-medium">No annotation layers match your filters</p><p className="text-muted-foreground mt-1 text-sm">Create a layer to annotate charts with events or intervals.</p><div className="mt-4 flex justify-center"><Button size="sm" onClick={openCreate}>Add layer</Button></div></div></td></tr>
                ) : rows.map((r) => (
                  <tr key={r.id} className={`group hover:bg-muted/40 ${selected.has(r.id) ? "bg-muted/60" : ""}`}>
                    ...
                    <td className="px-2 py-3"><button onClick={() => openEdit(r)} className="text-left text-sm font-medium hover:underline">{r.name}</button></td>
                    <td className="hidden px-2 py-2.5 sm:table-cell"><span className="text-muted-foreground text-xs">{r.description ?? "—"}</span></td>
                    <td className="px-2 py-2.5"><Badge variant="secondary" className="text-[11px] capitalize">{r.annotationType}</Badge></td>
                    <td className="hidden px-2 py-2.5 lg:table-cell"><span className="font-mono text-xs">{r.startField ?? "—"}</span></td>
                    <td className="hidden px-2 py-2.5 lg:table-cell"><span className="font-mono text-xs">{r.endField ?? "—"}</span></td>
        ...
        <p className="text-muted-foreground mt-3 text-xs">Data via <code className="bg-muted rounded px-1 py-0.5">/api/annotationlayers</code> — Postgres + Drizzle.</p>
      </div>
      <AnnotationEditor open={editorOpen} onClose={() => setEditorOpen(false)} initial={editing} onSave={handleSave} />
      <ConfirmDialog
        open={!!confirmRow}
      ...
      <ConfirmDialog
        ...
        title={`Delete ${selected.size} annotationlayers?`}
        description={`Delete ${selected.size} annotationlayers? This cannot be undone.`}
