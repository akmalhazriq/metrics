...
export default function AnnotationListPage() {
    ...
    try {
      ...
      const msg =
        e instanceof ApiError
          ...
          : e instanceof Error
            ? e.message
            : "We couldn't load annotation layers. Try refreshing.";
      setError(msg);
      showToast(msg);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    fetchList();
  }, [q, type, sortBy, sortDir, page, pageSize]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleBulkDelete = async () => {
    ...
    if (ok && !fail) showToast(`Deleted ${ok} annotation layers`);
      ...
      showToast(`Deleted ${ok} of ${ok + fail} annotation layers. ${fail} failed: ${lastErr}`);
  ...
  const handleSave = async (payload: Record<string, unknown>) => {
    if (editing) {
      const r = await fetch(`/api/annotationlayers/${editing.id as number}`, {
        method: "PUT",
      ...
      const r = await fetch("/api/annotationlayers", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(payload),
      });
      ...
      showToast(`Annotation layer "${String(payload.name)}" created`);
      setPage(1);
    }
    fetchList();
  };
    ...
    <AppShell>
      <div className="mx-auto max-w-[1280px] px-4 py-6 sm:px-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1 max-w-[60ch] text-sm leading-relaxed">
              Mark events and intervals on charts. Used by chart annotations to highlight releases,
              outages and targets.
            </p>
          </div>
          <Button size="sm" onClick={openCreate}>
            <StickyNote className="mr-1.5 h-3.5 w-3.5" /> Add layer
          </Button>
        </div>
                    ...
                    <td colSpan={7} className="px-6 py-16 text-center">
                      <div className="mx-auto max-w-sm">
                        <p className="text-sm font-medium">
                          No annotation layers match your filters
                        </p>
                        <p className="text-muted-foreground mt-1 text-sm">
                          Create a layer to annotate charts with events or intervals.
                        </p>
                  ...
                  rows.map((r) => (
                    ...
                    >
                      ...
                      <td className="hidden px-2 py-2.5 lg:table-cell">
                        <span className="font-mono text-xs">{r.startField ?? "—"}</span>
                      </td>
                      <td className="hidden px-2 py-2.5 lg:table-cell">
                        <span className="font-mono text-xs">{r.endField ?? "—"}</span>
                      </td>
                      <td className="px-2 py-3">
                        <div
        ...
        <p className="text-muted-foreground mt-3 text-xs">
          Data via <code className="bg-muted rounded px-1 py-0.5">/api/annotationlayers</code> —
          Postgres + Drizzle.
        </p>
      </div>
      <AnnotationEditor
