...
export default function ImportExportPage() {
  ...
  const handleExport = async () => {
    ...
    if (entities.length === 0) {
      showToast("Pick at least one entity type");
      return;
    }
    ...
    <AppShell>
      <div className="mx-auto max-w-[1280px] px-4 py-6 sm:px-6">
        ...
        <p className="text-muted-foreground mt-1 max-w-[60ch] text-sm leading-relaxed">
          Move dashboards, charts, datasets and databases as JSON. Import is scoped to JSON only —
          ZIP/YAML is deferred. Export hits{" "}
          <code className="bg-muted rounded px-1">/api/importexport/export</code> directly.
        </p>
            ...
            <div className="p-4">
              ...
              >
                ...
                <p className="text-muted-foreground text-xs">
                  Keys: dashboards, charts, datasets, databases — as from Export.
                </p>
                <input
                  ref={inputRef}
                  type="file"
              ...
              {parseError && (
                <p className="text-destructive mt-2 rounded bg-[var(--destructive)]/10 px-2 py-1 text-xs">
                  {parseError}
                </p>
              )}
              {parsed && (
                <div className="mt-3 rounded-md border border-dashed px-3 py-2">
                  ...
                  <pre className="mt-1 max-h-48 overflow-auto rounded bg-[var(--muted)] px-2 py-2 font-mono text-[11px]">
                    {JSON.stringify(parsed, null, 2).slice(0, 3000)}
                    {JSON.stringify(parsed).length > 3000 ? "\n… truncated" : ""}
                  </pre>
                </div>
              )}
              {importResult && (
              ...
              <div className="mt-4 flex justify-end">
                <Button size="sm" disabled={!parsed || importing} onClick={handleImport}>
            ...
            <div className="p-4">
              ...
              <div className="mt-2 grid gap-2">
                <label className="flex items-center gap-2 text-sm">
                  <Checkbox
                    checked={expDash}
                    onChange={(e) => setExpDash((e.target as HTMLInputElement).checked)}
                ...
                <label className="flex items-center gap-2 text-sm">
                  <Checkbox
                    checked={expChart}
                    onChange={(e) => setExpChart((e.target as HTMLInputElement).checked)}
                    aria-label="Charts"
                  />{" "}
                  Charts
                </label>
                <label className="flex items-center gap-2 text-sm">
                  <Checkbox
                    checked={expDataset}
                    onChange={(e) => setExpDataset((e.target as HTMLInputElement).checked)}
                    aria-label="Datasets"
                  />{" "}
                  Datasets
                </label>
                <label className="flex items-center gap-2 text-sm">
                  <Checkbox
                    checked={expDatabase}
                    onChange={(e) => setExpDatabase((e.target as HTMLInputElement).checked)}
              ...
              <label className="mt-4 block space-y-1.5">
                ...
                <Input
                  value={expIds}
                  onChange={(e) => setExpIds(e.target.value)}
                  placeholder="1, 2, 3, leave blank for all"
                  className="h-8 text-sm"
                />
                <span className="text-muted-foreground text-[11px]">
                  Filters dashboards/charts/datasets by id. Databases exports all.
                </span>
              </label>
              <div className="mt-4 flex justify-end">
                <Button size="sm" onClick={handleExport}>
                  <ArrowDownToLine className="mr-1.5 h-3.5 w-3.5" /> Export JSON
                </Button>
              </div>
              <div className="border-border bg-muted/20 mt-6 rounded-md border px-3 py-2">
                ...
                <p className="text-muted-foreground text-[11px] leading-relaxed">
                  Export then re-import to clone across environments. The file is also a valid
                  Import input — no transform needed. ZIP/YAML support is deferred.
                </p>
              </div>
            </div>
          </div>
        ...
        <p className="text-muted-foreground mt-3 text-xs">
          Import via{" "}
          <code className="bg-muted rounded px-1 py-0.5">POST /api/importexport/import</code>{" "}
          (transactional) · Export via{" "}
          <code className="bg-muted rounded px-1 py-0.5">
            GET /api/importexport/export?entities=…&ids=…
          </code>
          .
        </p>
      </div>
