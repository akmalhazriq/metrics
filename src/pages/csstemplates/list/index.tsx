...
import { ChevronDown, ChevronLeft, ChevronRight, ChevronsUpDown, Pencil, Search, SwatchBook, Trash2, X } from "lucide-react";
import { AppShell } from "@/components/layout/AppShell";
import { Button } from "@/components/ui/button";
import { Checkbox } from "@/components/ui/checkbox";
import { Input } from "@/components/ui/input";
import { ConfirmDialog } from "@/components/ui/confirm-dialog";
import { CssTemplateEditor } from "@/components/editors/CssTemplateEditor";
import { ApiError, fetchList as apiFetchList, mutate } from "@/lib/api";

type Row = { id: number; name: string; description: string | null; cssCode: string | null; createdAt: string; modifiedAt: string };

export default function CssTemplatesListPage() {
  const [q, setQ] = useState(""); const [sortBy, setSortBy] = useState("modified"); const [sortDir, setSortDir] = useState<"asc" | "desc">("desc");
  ...
  const fetchList = async () => {
    ...
    setError(null);
    try {
      const res = await apiFetchList<Row>("/api/csstemplates", { q: q || undefined, sortBy, sortDir, page, pageSize });
      setRows(res.data);
      setTotal(res.total);
    } catch (e) {
      const msg = e instanceof ApiError ? e.message : e instanceof Error ? e.message : "Could not load CSS templates";
      setError(msg);
      showToast(msg);
    } finally {
      setLoading(false);
  ...
  useEffect(() => { void fetchList(); }, [q, sortBy, sortDir, page, pageSize]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleDelete = async (row: Row) => {
    try {
      await mutate(`/api/csstemplates/${row.id}`, "DELETE");
      showToast("CSS template deleted");
      fetchList();
    } catch (e) {
  ...
  const handleBulkDelete = async () => {
    ...
    for (const id of ids) {
      try {
        await mutate(`/api/csstemplates/${id}`, "DELETE");
        ok++;
      } catch (e) {
        fail++;
    ...
    if (ok && !fail) showToast(`Deleted ${ok} CSS templates`);
    else if (ok && fail) showToast(`Deleted ${ok} of ${ok + fail} CSS templates — ${fail} failed: ${lastErr}`);
  ...
  const handleSave = async (payload: Record<string, unknown>) => {
    if (editing) {
      const r = await fetch(`/api/csstemplates/${editing.id as number}`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
      if (!r.ok) { const j = await r.json() as { error?: string }; showToast(j.error ?? "Save failed"); return; }
      showToast(`Template "${String(payload.name)}" saved`);
    } else {
      const r = await fetch("/api/csstemplates", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
      if (!r.ok) { const j = await r.json() as { error?: string }; showToast(j.error ?? "Create failed"); return; }
      showToast(`Template "${String(payload.name)}" created`); setPage(1);
    }
    fetchList();
  };
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-[1280px] px-4 py-6 sm:px-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <div className="bg-muted text-muted-foreground grid h-7 w-7 place-items-center rounded-md"><SwatchBook className="h-4 w-4" /></div>
              <h1 className="text-[22px] font-semibold tracking-tight">CSS Templates</h1>
              <span className="bg-muted text-muted-foreground rounded-full px-2 py-0.5 text-xs font-medium">{total}</span>
            </div>
            <p className="text-muted-foreground mt-1 max-w-[60ch] text-sm leading-relaxed">Reusable CSS applied to dashboards from dashboard properties. Keep it to layout and chrome overrides.</p>
          </div>
          <Button size="sm" onClick={openCreate}><SwatchBook className="mr-1.5 h-3.5 w-3.5" /> Add template</Button>
        </div>
        ...
        <div className="border-border bg-card mt-6 rounded-lg border">
          ...
          <div className="border-border flex items-center gap-2 border-t px-3 py-2">
            <span className="text-muted-foreground text-xs">{loading ? "Loading…" : `${total} templates`}</span>
            {q && <button onClick={() => { setQ(""); setPage(1); }} className="border-input bg-background ml-auto inline-flex items-center gap-1 rounded-md border px-2 py-1 text-xs font-medium"><X className="h-3 w-3" /> Clear</button>}
          </div>
          {selected.size > 0 && (
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-border divide-y">
                {loading ? Array.from({ length: 4 }).map((_, i) => (
                  <tr key={i} className="animate-pulse"><td className="px-3 py-3"><span className="bg-muted block h-3 w-3 rounded" /></td><td className="px-2 py-3"><span className="bg-muted block h-3 w-24 rounded" /></td><td className="px-2 py-3"><span className="bg-muted block h-3 w-32 rounded" /></td><td /></tr>
                )) : rows.length === 0 ? (
                  <tr><td colSpan={4} className="px-6 py-16 text-center"><div className="mx-auto max-w-sm"><p className="text-sm font-medium">No CSS templates match your filters</p><p className="text-muted-foreground mt-1 text-sm">Create a template to style dashboards.</p><div className="mt-4 flex justify-center"><Button size="sm" onClick={openCreate}>Add template</Button></div></div></td></tr>
                ) : rows.map((r) => (
                  <tr key={r.id} className={`group hover:bg-muted/40 ${selected.has(r.id) ? "bg-muted/60" : ""}`}>
                    ...
                    <td className="px-2 py-3"><button onClick={() => openEdit(r)} className="text-left text-sm font-medium hover:underline">{r.name}</button><div className="text-muted-foreground hidden max-w-[40ch] truncate font-mono text-[11px] sm:block">{r.cssCode?.slice(0, 80) ?? "—"}{r.cssCode && r.cssCode.length > 80 ? "…" : ""}</div></td>
        ...
        <p className="text-muted-foreground mt-3 text-xs">Data via <code className="bg-muted rounded px-1 py-0.5">/api/csstemplates</code> — Postgres + Drizzle.</p>
      </div>
      <CssTemplateEditor open={editorOpen} onClose={() => setEditorOpen(false)} initial={editing} onSave={handleSave} />
      <ConfirmDialog
        open={!!confirmRow}
      ...
      <ConfirmDialog
        ...
        title={`Delete ${selected.size} csstemplates?`}
        description={`Delete ${selected.size} csstemplates? This cannot be undone.`}
