...
export default function CssTemplatesListPage() {
  ...
  const fetchList = async () => {
    ...
    setError(null);
    try {
      const res = await apiFetchList<Row>("/api/csstemplates", {
        q: q || undefined,
        sortBy,
        sortDir,
        page,
      ...
      const msg =
        e instanceof ApiError
          ...
          : e instanceof Error
            ? e.message
            : "We couldn't load CSS templates. Try refreshing.";
      setError(msg);
      showToast(msg);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    void fetchList();
  }, [q, sortBy, sortDir, page, pageSize]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleBulkDelete = async () => {
    ...
    if (ok && !fail) showToast(`Deleted ${ok} CSS templates`);
      ...
      showToast(`Deleted ${ok} of ${ok + fail} CSS templates. ${fail} failed: ${lastErr}`);
  ...
  const handleSave = async (payload: Record<string, unknown>) => {
    if (editing) {
      const r = await fetch(`/api/csstemplates/${editing.id as number}`, {
        method: "PUT",
      ...
      const r = await fetch("/api/csstemplates", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(payload),
      });
      ...
      showToast(`Template "${String(payload.name)}" created`);
      setPage(1);
    }
    fetchList();
  };
    ...
    <AppShell>
      <div className="mx-auto max-w-[1280px] px-4 py-6 sm:px-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1 max-w-[60ch] text-sm leading-relaxed">
              Reusable CSS applied to dashboards from dashboard properties. Keep it to layout and
              chrome overrides.
            </p>
          </div>
          <Button size="sm" onClick={openCreate}>
            <SwatchBook className="mr-1.5 h-3.5 w-3.5" /> Add template
          </Button>
        </div>
        ...
        <div className="border-border bg-card mt-6 rounded-lg border">
          ...
          <div className="border-border flex items-center gap-2 border-t px-3 py-2">
            <span className="text-muted-foreground text-xs">
              {loading ? "Loading…" : `${total} templates`}
            </span>
            {q && (
              <button
                onClick={() => {
                  ...
                  <tr>
                    <td colSpan={4} className="px-6 py-16 text-center">
                      <div className="mx-auto max-w-sm">
                        <p className="text-sm font-medium">No CSS templates match your filters</p>
                        <p className="text-muted-foreground mt-1 text-sm">
                          Create a template to style dashboards.
                        </p>
                  ...
                  rows.map((r) => (
                    ...
                    >
                      ...
                      <td className="px-2 py-3">
                        ...
                        <div className="text-muted-foreground hidden max-w-[40ch] truncate font-mono text-[11px] sm:block">
                          {r.cssCode?.slice(0, 80) ?? "—"}
                          {r.cssCode && r.cssCode.length > 80 ? "…" : ""}
                        </div>
                      </td>
        ...
        <p className="text-muted-foreground mt-3 text-xs">
          Data via <code className="bg-muted rounded px-1 py-0.5">/api/csstemplates</code> —
          Postgres + Drizzle.
        </p>
      </div>
      <CssTemplateEditor
