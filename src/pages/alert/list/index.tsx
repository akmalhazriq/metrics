...
import { Bell, ChevronDown, ChevronLeft, ChevronRight, ChevronsUpDown, Pencil, Search, Send, Trash2, X } from "lucide-react";
...
export default function AlertListPage() {
  ...
  const fetchList = async () => {
    ...
    try {
      ...
      const msg = e instanceof ApiError ? e.message : e instanceof Error ? e.message : "Could not load Alerts";
      setError(msg);
      showToast(msg);
    } finally {
      setLoading(false);
  ...
  const handleToggle = async (row: AlertRow) => {
    await fetch(`/api/alerts/${row.id}`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ active: !row.active }) });
    fetchList();
  };
  const handleBulkDelete = async () => {
    ...
    else if (ok && fail) showToast(`Deleted ${ok} of ${ok + fail} alerts — ${fail} failed: ${lastErr}`);
  ...
  const handleTest = async (rowOrPayload: Record<string, unknown>) => {
    ...
    if (id) {
      try {
        ...
        showToast(j.message ?? "Alert logic validated — delivery not configured for this placeholder phase");
      } catch { showToast("Test failed"); }
      return;
    }
    const n = Array.isArray(rowOrPayload.recipients) ? (rowOrPayload.recipients as string[]).length : String(rowOrPayload.recipients ?? "").split(",").filter(Boolean).length;
    showToast(`Test alert sent to ${n || 1} recipient${n === 1 ? "" : "s"} — delivery not configured for this placeholder phase`);
  };
  const handleSave = async (payload: Record<string, unknown>) => {
    if (editing) {
      const r = await fetch(`/api/alerts/${editing.id}`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
  ...
  const openEdit = (row: AlertRow) => { setEditing(row); setEditorOpen(true); };
      ...
      <div className="mx-auto max-w-[1280px] px-4 py-6 sm:px-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <div className="bg-muted text-muted-foreground grid h-7 w-7 place-items-center rounded-md"><Bell className="h-4 w-4" /></div>
            ...
            <p className="text-muted-foreground mt-1 max-w-[60ch] text-sm leading-relaxed">SQL checks that notify when a threshold is crossed. Runs on cron; delivery via email, Slack, or webhook.</p>
          </div>
          <Button size="sm" onClick={openCreate}><Bell className="mr-1.5 h-3.5 w-3.5" /> Add alert</Button>
        </div>
        ...
        <div className="border-border bg-card mt-6 rounded-lg border">
          ...
          <div className="border-border flex flex-wrap items-center gap-2 border-t px-3 py-2">
            <span className="text-muted-foreground hidden text-xs sm:inline">Cron like <code className="bg-muted rounded px-1">0 9 * * MON</code> = Every Monday at 9am</span>
            <div className="ml-auto flex items-center gap-2 text-xs">
              <span className="text-muted-foreground">{loading ? "Loading…" : `${total} alerts`}</span>
              {(q || status !== "all" || active !== "all") && <button onClick={() => { setQ(""); setStatus("all"); setActive("all"); setPage(1); }} className="border-input bg-background hover:bg-accent inline-flex items-center gap-1 rounded-md border px-2 py-1 font-medium"><X className="h-3 w-3" /> Clear</button>}
            </div>
          </div>
          {selected.size > 0 && (
            <div className="border-border bg-muted/50 flex items-center gap-2 border-t px-3 py-2">
              ...
              <button onClick={() => setSelected(new Set())} className="text-muted-foreground hover:text-foreground ml-auto text-xs font-medium">Clear</button>
            </div>
          )}
        </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-border divide-y">
                {loading ? Array.from({ length: 4 }).map((_, i) => (
                  <tr key={i} className="animate-pulse"><td className="px-3 py-3"><span className="bg-muted block h-3 w-3 rounded" /></td><td className="px-2 py-3"><span className="bg-muted block h-3 w-32 rounded" /></td><td colSpan={7} className="px-2 py-3"><span className="bg-muted block h-3 w-24 rounded" /></td></tr>
                )) : rows.length === 0 ? (
                  <tr><td colSpan={9} className="px-6 py-16 text-center"><div className="mx-auto max-w-sm"><p className="text-sm font-medium">No alerts match your filters</p><p className="text-muted-foreground mt-1 text-sm">Create one to get notified when a SQL condition is met.</p><div className="mt-4 flex justify-center"><Button size="sm" onClick={openCreate}>Add alert</Button></div></div></td></tr>
                ) : rows.map((r) => (
                  <tr key={r.id} className={`group hover:bg-muted/40 ${selected.has(r.id) ? "bg-muted/60" : ""}`}>
                    ...
                    <td className="px-2 py-3"><button onClick={() => openEdit(r)} className="text-left text-sm font-medium hover:underline">{r.name}</button><div className="text-muted-foreground hidden max-w-[28ch] truncate text-[11px] sm:block">{r.sqlQuery ?? "—"}</div></td>
                    <td className="hidden px-2 py-2.5 sm:table-cell"><Badge variant="secondary" className="text-[11px]">{r.type}</Badge></td>
                    <td className="hidden px-2 py-2.5 lg:table-cell"><span className="text-xs">{r.trigger}</span></td>
                    ...
                    <td className="px-2 py-2.5"><label className="inline-flex cursor-pointer items-center gap-1.5"><input type="checkbox" checked={r.active} onChange={() => handleToggle(r)} className="accent-primary h-3.5 w-3.5" /><span className="text-xs">{r.active ? "On" : "Off"}</span></label></td>
                      ...
                      <div className="relative flex justify-end" ref={openMenu === r.id ? menuRef : undefined}>
                        ...
                        {openMenu === r.id && (
                          <div className="border-border bg-popover absolute top-8 right-0 z-20 w-56 rounded-md border p-1 shadow-lg">
                            ...
                            <button onClick={() => { setOpenMenu(null); handleToggle(r); }} className="hover:bg-accent flex w-full items-center gap-2 rounded px-2 py-1.5 text-xs"><Bell className="h-3.5 w-3.5" /> {r.active ? "Pause" : "Enable"}</button>
        ...
        <p className="text-muted-foreground mt-3 text-xs">Data via <code className="bg-muted rounded px-1 py-0.5">/api/alerts</code> — Postgres + Drizzle.</p>
      </div>
      <AlertReportEditor mode="alert" open={editorOpen} onClose={() => setEditorOpen(false)} initial={editing as unknown as Record<string, unknown>} onSave={handleSave} onTest={handleTest} />
      <ConfirmDialog
        open={!!confirmRow}
